...
public class CallDetailActivity extends Activity implements ProximitySensorAware {
    ...
    private final View.OnClickListener mPrimaryActionListener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
             ...
             * add by xuhong.tian
	         */
            Intent callIntent = ((ViewEntry) view.getTag()).primaryIntent;
			int call_mode = (Integer) callIntent.getExtra(
					privateCallandSmsUtil.CALL_MODE_FLAG, -1);
			String call_number = (String) callIntent.getExtra(
					privateCallandSmsUtil.CALL_PRIVATE_NUMBER, "");

			Log.d(TAG, "call_mode="+call_mode+"call_number="+call_number);
			
			if (call_mode != Calls.PUBLIC_CALL_FLAG) {

				Intent phoneIntent =privateCallandSmsUtil.getForwardPrivatePhone("", call_number, call_mode);
    ...
    private final View.OnClickListener mSecondaryActionListener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
			...
			Intent smsIntent = ((ViewEntry) view.getTag()).secondaryIntent;
			int call_mode = (Integer) smsIntent.getExtra(
					privateCallandSmsUtil.CALL_MODE_FLAG, -1);
			String call_number = (String) smsIntent.getExtra(
					privateCallandSmsUtil.CALL_PRIVATE_NUMBER, "");
			
			Log.d(TAG, "call_mode="+call_mode+"call_number="+call_number);
			if (call_mode != Calls.PUBLIC_CALL_FLAG) {
				
				Intent privatesmsIntent = privateCallandSmsUtil.getForwardPrivateSms("", call_number, call_mode);
				startActivity(privatesmsIntent);
			} else {
...
/*            bindContactPhotoAction(intent, R.drawable.ic_add_contact_holo_dark,
					...
					/**add by xuhong.tian remove the bind on 11.11**/
        }
        ...
        public Loader<Contact> onCreateLoader(int id, Bundle args) {
            ...
            if (contactUri == null) {
                Log.d(TAG, "No contact lookup uri provided.");
            }
    ...
    private void updateData(final Uri... callUris) {
        class UpdateContactDetailsTask extends AsyncTask<Void, Void, PhoneCallDetails[]> {
            ...
            public void onPostExecute(PhoneCallDetails[] details) {
                ...
                final int call_mode = firstDetails.callMode;
                ...
                if (contactUri != null && !UriUtils.isEncodedContactUri(contactUri)) {
					...
					if (call_mode != Calls.PUBLIC_CALL_FLAG) {
						mainActionIntent = new Intent(Intent.ACTION_INSERT,
						...
						mainActionIntent.putExtra(Intents.Insert.ACCOUNT,
								account);
						mainActionIntent.putExtra(Insert.PHONE, mNumber);
						Bundle extras = new Bundle();
						extras.putInt("public_create", 1);
						mainActionIntent.putExtras(extras);

					} else {

						mainActionIntent = new Intent(
                ...
                /**add by xuhong.tian add  the bind on 11.11**/
                	Intent mainActionIntent1=null;
                	Log.d(TAG, "canPlaceCallsTo=="+canPlaceCallsTo+"call_mode=="+call_mode);
					if (call_mode != Calls.PUBLIC_CALL_FLAG) {
						mainActionIntent1 = new Intent(Intent.ACTION_INSERT,
								Contacts.CONTENT_URI);
						AccountWithDataSet account = new AccountWithDataSet(
								getString(R.string.show_privatecontacts),
								PhoneAccountType.PRIVATE_ACCOUNT_TYPE, null);
						mainActionIntent1.putExtra(Intents.Insert.ACCOUNT,
								account);
						mainActionIntent1.putExtra(Insert.PHONE, mNumber);
						Bundle extras = new Bundle();
						extras.putInt("public_create", 1);
						mainActionIntent1.putExtras(extras);

					} else {

						mainActionIntent1 = new Intent(
								Intent.ACTION_INSERT_OR_EDIT);
						mainActionIntent1.setType(Contacts.CONTENT_ITEM_TYPE);
						mainActionIntent1.putExtra(Insert.PHONE, mNumber);
					}
                	
                	   bindContactPhotoAction(mainActionIntent1, R.drawable.ic_add_contact_holo_dark,
                               getString(R.string.description_add_contact));
                }
                ...
                if (canPlaceCallsTo) {
					 ...
					 * add by xuhong.tian
					 */
                    Intent privilege= CallUtil.getCallIntent(mNumber);
                    privilege.putExtra(privateCallandSmsUtil.CALL_MODE_FLAG, call_mode);
                    privilege.putExtra(privateCallandSmsUtil.CALL_PRIVATE_NUMBER, mNumber);
                     ...
                     * add by xuhong.tian
                     */
                    Intent seccondIntent=new Intent(Intent.ACTION_SENDTO,
                            Uri.fromParts("sms", mNumber, null));
                    seccondIntent.putExtra(privateCallandSmsUtil.CALL_MODE_FLAG, call_mode);
                    seccondIntent.putExtra(privateCallandSmsUtil.CALL_PRIVATE_NUMBER, mNumber);
    ...
    private PhoneCallDetails getPhoneCallDetailsForUri(Uri callUri) {
        ...
        try {
             ...
             * add by xuhong.tian
             */
            int call_mode = callCursor.getInt(CALL_MODE);
            ...
            ContactInfo info =
                    ...
                    if(SprdUtils.UNIVERSE_UI_SUPPORT){
                        return new PhoneCallDetails(number, numberPresentation, formattedNumber, countryIso, geocode,
                                ...
                                numberLabel, lookupUri, photoUri, phoneId, videoCall,iccId,call_mode);
                    } else {
                        return new PhoneCallDetails(number, numberPresentation, formattedNumber, countryIso, geocode,
                                new int[]{ callType }, date, duration,
                                nameText, numberType, numberLabel, lookupUri, photoUri, phoneId, videoCall,call_mode);
                    }
                    /* @} */
        } finally {
            if (callCursor != null) {
    ...
    private void configureCallButton(ViewEntry entry) {
         ...
         */
        int call_mode = 0;
		call_mode = (Integer) entry.secondaryIntent.getExtra(
				privateCallandSmsUtil.CALL_MODE_FLAG, -1);
        if(SprdUtils.UNIVERSE_UI_SUPPORT){
            smsIcon = (ImageView) convertView.findViewById(R.id.call_and_sms_icon);
        ...
        if (entry.secondaryIntent != null) {
        	smsIcon.setOnClickListener(mSecondaryActionListener);
        	
			if (call_mode != Calls.PUBLIC_CALL_FLAG) {
				//smsIcon.setBackgroundResource(R.drawable.private_contacats_sms_icon);
				
/*		        Drawable background = getResources().getDrawable(R.drawable.private_contacats_sms_icon);
		        smsIcon.setBackgroundDrawable(background);*/
			} else {
				if (!SprdUtils.UNIVERSE_UI_SUPPORT)
					smsIcon.setImageResource(entry.secondaryIcon);
			}
            smsIcon.setVisibility(View.VISIBLE);
            smsIcon.setTag(entry);
            smsIcon.setContentDescription(entry.secondaryDescription);
            if(!SprdUtils.UNIVERSE_UI_SUPPORT)
        ...
        if(SprdUtils.UNIVERSE_UI_SUPPORT){
            ...
            if (TextUtils.isEmpty(entry.label)) {
    			...
    			if (call_mode != Calls.PUBLIC_CALL_FLAG) {
    				label.setText(getString(R.string.private_number));
    			} else {
