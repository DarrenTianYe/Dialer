...
public class DialerDatabaseHelper extends SQLiteOpenHelper {
    ...
    public static interface PhoneQuery {
       ...
       public static final String[] PROJECTION_SPRD = new String[] {
           Phone._ID,
           Phone.DISPLAY_NAME,
           Phone.PHOTO_ID,
           Phone.NUMBER,
    ...
    public ArrayList<ContactNumber>  getLooseMatches(String query,
         ...
         * @Orig:if (inUpdate) {
         * @ { */
        if (inUpdate && !SprdUtils.UNIVERSE_UI_SUPPORT) {
            /* @}*/
        ...
        int columnDataId = 0;
        int columnDisplayNamePrimary = 1;
        int columnPhotoId = 2;
        int columnNumber = 3;
        int columnId = 4;
        int columnLookupKey = 5;
        if (DEBUG) {
        ...
        if(SprdUtils.UNIVERSE_UI_SUPPORT){
            ...
            if (!TextUtils.isEmpty(query)) {
                Builder builder = Uri.parse("content://call_log/calls/contacts").buildUpon();
                Uri uri = builder.appendPath(query).build();
                cursor = mContext.getContentResolver().query(uri, PhoneQuery.PROJECTION_SPRD, null,
                        null,
        ...
        /* SPRD: return directly when cursor is null @{ */
        if(cursor == null){
            return result;
        }
        /* @} */
        ...
        try {
            ...
            while ((cursor.moveToNext()) && (counter < MAX_ENTRIES)) {
                ...
                if (nameMatches || numberMatches || SprdUtils.UNIVERSE_UI_SUPPORT) {//SPRD:add for match search index
