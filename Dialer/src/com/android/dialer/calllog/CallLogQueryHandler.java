...
public class CallLogQueryHandler extends NoNullCursorAsyncQueryHandler {
    ...
    public void fetchCalls(int callType, long newerThan) {
        fetchCalls(callType, CallLogSetting.TYPE_ALL, newerThan);
    }

    /*
     * SPRD:
     */
    public void fetchCalls(int callType, int showType, long newerThan) {
        cancelFetch();
        int requestId = newCallsRequest();
        fetchCalls(QUERY_CALLLOG_TOKEN, requestId, callType, showType, false /* newOnly */, newerThan, null);
    }

    public void fetchCalls(int callType, int showType, long newerThan, Context context) {
        cancelFetch();
        int requestId = newCallsRequest();
        fetchCalls(QUERY_CALLLOG_TOKEN, requestId, callType, showType, false /* newOnly */, newerThan, context);
    }
     ...
     * @orig fetchCalls(int)
     */
    public void fetchCalls(int callType, int showType) {
        fetchCalls(callType, showType, 0);
    }

    public void fetchCalls(int callType, int showType, Context context) {
        fetchCalls(callType, showType, 0, context);
    }
    ...
    public void fetchSimCalls(int phoneId){
        cancelFetch();
       // invalidate();
        //fetchCalls(QUERY_NEW_CALLS_TOKEN, true /*isNew*/, false /*voicemailOnly*/ ,phoneId);
        //fetchCalls(QUERY_OLD_CALLS_TOKEN, false /*isNew*/, false /*voicemailOnly*/ ,phoneId);
    }
    public void fetchModeCalls(int mode){
        cancelFetch();
      //  invalidate();
        //fetchModeCalls(QUERY_NEW_CALLS_TOKEN, true /*isNew*/, false /*voicemailOnly*/ ,mode);
       // fetchModeCalls(QUERY_OLD_CALLS_TOKEN, false /*isNew*/, false /*voicemailOnly*/ ,mode);
    }
    ...
    /*
     * SPRD: modify
     * @orig fetchCalls(int, int, int, boolean, long)
     */
    private void fetchCalls(int token, int requestId, int callType, int showType, boolean newOnly,
            long newerThan, Context context) {
    	Log.d(TAG, "token=="+token+"requestid="+requestId+"calltype="+callType+"showType="+showType+"newOnly="+newOnly+"newerThan="+newerThan);
        ...
        /* SPRD: filter show type @{ */
        if(showType > CallLogSetting.TYPE_ALL){
            if (where.length() > 0) {
                where.append(" AND ");
            }
            if(SprdUtils.UNIVERSE_UI_SUPPORT && context != null){
                String iccId = SprdUtils.getIccIdByPhoneId(showType, context);
                if(iccId != null){
                    where.append(String.format("(%s = ?)", Calls.ICC_ID));
                    selectionArgs.add(iccId);
                } else {
                    where.append(String.format("(%s = ?)", Calls.PHONE_ID));
                    selectionArgs.add(Integer.toString(showType));
                }
			} else {
				// where.append(String.format("(%s = ?)", Calls.PHONE_ID));
				// selectionArgs.add(Integer.toString(showType));

				if(0==showType){// all call log 
				...
				}else if(1 == showType)// sim call log 
				{
					where.append(String.format("(%s = ?)", Calls.PHONE_ID));
					selectionArgs.add("0");
					
				}else if(2 == showType){// pdt call log 
					where.append(String.format("(%s = ?)", Calls.CALL_MODE));
					selectionArgs.add("0");
				}
