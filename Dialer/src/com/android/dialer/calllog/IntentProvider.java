...
public abstract class IntentProvider {
    ...
    public static IntentProvider getReturnCallIntentProvider(final String number,final int call_mode) {
        return new IntentProvider() {
            @Override
            public Intent getIntent(Context context) {
            	Intent intent = CallUtil.getCallIntent(number);
                intent.putExtra(privateCallandSmsUtil.CALL_PRIVATE_NUMBER, number);
                intent.putExtra(privateCallandSmsUtil.CALL_MODE_FLAG, call_mode);	
                return intent;
            }
        };
    }
    ...
    public static IntentProvider getCallDetailIntentProvider(
            final Cursor cursor, final int position, final long id, final int groupSize, final int call_mode) {
        return new IntentProvider() {
            @Override
            public Intent getIntent(Context context) {
                if (cursor.isClosed()) {
                ...
                intent.putExtra(privateCallandSmsUtil.CALL_PRIVATE_NUMBER, cursor.getInt(CallLogQuery.NUMBER));
                intent.putExtra(privateCallandSmsUtil.CALL_MODE_FLAG, cursor.getInt(CallLogQuery.CALL_MODE));
                /* @} */
                if (groupSize > 1) {
    ...
    /* SPRD: add for Universe UI { */
    public static IntentProvider getCallDetailIntentProvider(
            final Cursor cursor, final int position, final long id, final int groupSize, final Uri photoUri, final int call_mode) {
        return new IntentProvider() {
            @Override
            public Intent getIntent(Context context) {
                if (cursor.isClosed()) {
                ...
                intent.putExtra(CallLogAdapter.NUMBER, cursor.getString(CallLogQuery.NUMBER));
                intent.putExtra(CallLogAdapter.NAME, cursor.getString(CallLogQuery.CACHED_NAME));
                intent.putExtra(CallLogAdapter.PHOTO_URI, photoUri);
                intent.putExtra(CallLogAdapter.CACHED_LOOKUP_URI, cursor.getString(CallLogQuery.CACHED_LOOKUP_URI));
                intent.putExtra(privateCallandSmsUtil.CALL_PRIVATE_NUMBER, cursor.getString(CallLogQuery.NUMBER));
                intent.putExtra(privateCallandSmsUtil.CALL_MODE_FLAG, cursor.getString(CallLogQuery.CALL_MODE));
