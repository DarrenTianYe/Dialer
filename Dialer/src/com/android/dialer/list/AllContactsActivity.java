	...
	private final OnPhoneNumberPickerActionListener mPhoneNumberPickerActionListener = new OnPhoneNumberPickerActionListener() {
		...
		public void onPickPhoneNumberAction(Uri dataUri, String number,
			...
			if (Integer.valueOf(mode) != 0) {
				startActivity(privateCallandSmsUtil.getForwardPrivatePhone("", number, Integer.valueOf(mode)));
		...
		public void onShortcutIntentCreated(Intent intent) {
			Log.d(TAG, "Unsupported intent has come (" + intent
					+ "). Ignoring.");
		}

		@Override
		public void onHomeInActionBarSelected() {
			// {@link PhoneNumberPickerFragment handles onClick on the home
			// button
			// and performs the callback here. This means we don't have to
			// handle it
